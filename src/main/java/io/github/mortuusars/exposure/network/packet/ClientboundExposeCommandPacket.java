...
public record ClientboundExposeCommandPacket(int size, boolean storeOnServer) {
    ...
    public static ClientboundExposeCommandPacket fromBuffer(FriendlyByteBuf buffer) {
        return new ClientboundExposeCommandPacket(buffer.readInt(), buffer.readBoolean());
    }

    @SuppressWarnings("UnusedReturnValue")
    public boolean handle(Supplier<NetworkEvent.Context> contextSupplier) {
        DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () -> contextSupplier.get().enqueueWork(
                () -> ClientPacketsHandler.exposeScreenshot(size, storeOnServer)));
